...
export function getTokenData(tokenAddress?: string) {
  ...
  if (!tokenDataCache) {
    ...
    while (i < buf.length) {
      ...
      // TODO a better way of handling config.chainId
  ...
  if (!tokenData) {
    ...
    console.debug(`No token data found for ${tokenAddress}`)
    return null
  }
  return tokenData
}
